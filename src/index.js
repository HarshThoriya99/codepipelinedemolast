...
exports.handler = (event, context, callback) => {
  ...
  const resourceOptions =
    requestType === constants.DELETE ? {} : event.ResourceProperties.Options;

  if (event.LogicalResourceId != "WebsiteDeployment") {
    return sendCloudFormationResponse(constants.FAILED, {
      message: `Invalid LogicalResourceId: ${event.LogicalResourceId}`,
    });
  }
  ...
  switch (requestType) {
    ...
    default:
      return sendCloudFormationResponse(constants.FAILED, {
        message: `Invalid request type ${requestType}`,
      });
  }
  ...
  function cleanBucket(resourceId) {
    if (!resourceId || resourceId == "") {
      return sendCloudFormationResponse(constants.FAILED, {
        message: `Invalid physical resource id: ${resourceId}`,
      });
    }
    ...
    s3.listObjects({ Bucket: bucketName }, function (err, data) {
      if (err) {
        return sendCloudFormationResponse(constants.FAILED, {
          message: `Could not list bucket objects: ${err}`,
        });
      }
      ...
      for (var i = 0; i < items.length; i += 1) {
        ...
        s3.deleteObject(deleteParams, function (err, data) {
          if (err) {
            return sendCloudFormationResponse(constants.FAILED, {
              message: `Could not delete object: ${items[i].Key}`,
            });
          }
        });
      }
    ...
    return sendCloudFormationResponse(
      constants.SUCCESS,
      { message: "OK" },
      resourceId
    );
  ...
  function uploadArtifacts(resourceOptions) {
    if (
      !resourceOptions ||
      !resourceOptions["SourceBucket"] ||
      !resourceOptions["SourceArtifact"] ||
      !resourceOptions["DestinationBucket"]
    ) {
      return sendCloudFormationResponse(constants.FAILED, {
        message:
          "Missing required options: SourceBucket, SourceArtifact, DestinationBucket",
      });
    }
    ...
    const physicalResourceId =
      "Deployment::" + resourceOptions.DestinationBucket;
    ...
    // get source artifact
    s3.getObject(
      { Bucket: sourceBucket, Key: sourceArtifact },
      function (err, data) {
        if (err) {
          return sendCloudFormationResponse(constants.FAILED, {
            message: `Could not fetch artifact: ${sourceBucket}/${sourceArtifact}: ${err}`,
          });
        }

        try {
          fs.writeFileSync(tmpSourceArtifact, data.Body, {
            encoding: "binary",
          });
        } catch (ex) {
          return sendCloudFormationResponse(constants.FAILED, {
            message: `Could not save artifact to disk: ${ex}`,
          });
        }
        ...
        zipEntries.forEach(function (zipEntry) {
          if (zipEntry.entryName == "package.zip") {
            ...
            try {
              ...
              return sendCloudFormationResponse(constants.FAILED, {
                message: `Could not save package to disk: ${ex}`,
              });
            }
          }
        });
        ...
        if (!packageFound) {
          return sendCloudFormationResponse(constants.FAILED, {
            message: "Could not package.zip in artifact",
          });
        }
        ...
        packageEntries.forEach(function (entry) {
          ...
          asyncTasks.push(function (callback) {
            ...
            s3.putObject(s3FileProperties, function (err, data) {
              if (err) callback(err, entry.entryName);
              else callback(null, data.Key);
            });
          });
        });
        ...
        async.parallel(asyncTasks, function (err, result) {
          if (err)
            return sendCloudFormationResponse(constants.FAILED, {
              message: `Error while uploading ${result} to destination bucket: ${err}`,
            });
          else
            return sendCloudFormationResponse(
              constants.SUCCESS,
  ...
  function sendCloudFormationResponse(
    responseStatus,
    responseData,
    physicalResourceId
  ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(() =>
        callback(
          responseStatus === constants.FAILED ? responseStatus : null,
          responseData
        )
      )
      .catch(callback);
